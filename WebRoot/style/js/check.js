...
function insertTip(obj, val, location, msg, tclass){
    ...
    //根据tclass确定提示图标
    var tipIconClass = "glyphicon ";
    var checkBackClass = "has-checkback ";
    if(tclass == 'correct'){
        tipIconClass += "glyphicon-ok";
        checkBackClass += "has-correct";

    }else{
        tipIconClass += "glyphicon-remove";
        checkBackClass += "has-error";
    }
    var tipIcon = '<i class="sdy-tip ' + tipIconClass + '"></i>';
    var span = '<span class="sdy-tip ' + tclass + '">' + msg + '</span>';
    var locationSpan = '<span class="sdy-tip location ' + tclass + '">' + tipIcon + msg + '</span>';
    ...
    if(location){
        ...
        loc.html(locationSpan);
    }else{
        if($(obj).parents(".check-group").next(".sdy-tip").length > 0){
            $(obj).parents(".check-group").find(".sdy-tip").replaceWith(tipIcon);
            $(obj).parents(".check-group").next(".sdy-tip").replaceWith(span);
        }else{
            $(obj).after(tipIcon);
            $(obj).parents(".check-group").after(span);
        }
        $(obj).parents(".check-group").attr("class", "check-group " + checkBackClass);
    }   
}
...
$(function(){
	$(document).on("keyup blur", "input[type=text][check],  input[type=password][check], textarea[check]", function() {
    	checkVal(this);
    });

	$(document).on("change", "input[type=checkbox][check], input[type=radio][check], select[check]", function() {
        checkVal(this);
    });
