...
/***
 * 
 * @描述 : 基类Dao（Repository），所有Dao（Repository）需继承
 * @创建者：fengdi
...
public abstract class BaseHibernateDao extends HibernateDaoSupport implements BaseDao {
	...
	public Page list(final Page p, final String name) {
		return (Page) hibernateTemplate.execute(new HibernateCallback<Page>() {
			public Page doInHibernate(Session session) throws HibernateException, SQLException {
				...
				Page temp = queryByPage(hql, parmvalues, (p.getCurrentPageNumber() - 1) * p.getItemsPerPage(),
	...
	protected Page queryByPage(final String listHql, final List<? extends Object> params, final Integer firstResult,
			final Integer maxResults, final Long totalCount, Session session) {
		...
		if (totalCount > 0) {
			...
			countHql = new StringBuffer("select count(*) from (").append(queryTranslator.getSQLString())
